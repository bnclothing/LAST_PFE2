...
if (!empty($selectedGroupID)) {
    $stmt = $db->prepare("SELECT * FROM (SELECT m.*, u.FIRST_NAME, u.LAST_NAME FROM message m JOIN users u ON m.ID_USER = u.ID_USER WHERE m.ID_GROUP_DISCUSSION = :group_id ORDER BY m.TIMESTAMP DESC LIMIT 15) AS sub ORDER BY sub.TIMESTAMP ASC");
